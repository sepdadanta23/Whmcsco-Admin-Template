...
<h2 class="ticket-subject">
    ...
    <a href="supporttickets.php#" onclick="$('#ticketstatus').val('Closed');$('#ticketstatus').trigger('change');return false" class="btn btn-danger">{$_ADMINLANG.global.close}</a>
</h2>
        ...
        {foreach $replies as $reply}
            <div class="reply {if $reply.note} note{elseif $reply.admin} staff{/if}">
                ...
                <div class="hidemin">
                    ...
                    <div class="tools">
                        <div class="ticket-manage {if $reply.id}r{$reply.id}{else}t{$ticketid}{/if}">
                            <img src="../assets/img/spinner.gif" width="16" height="16" class="saveSpinner" style="display: none" />
                            {if !$reply.note}
                            <a type="button" onclick="editTicket('{if $reply.id}r{$reply.id}{else}t{$ticketid}{/if}')"><i class="fa fa-pencil fa-fw"></i></a>
                            {/if}
                            {if $deleteperm}
                            <a type="button" onclick="{if $reply.id}{if $reply.note}doDeleteNote('{$reply.id}');{else}doDeleteReply('{$reply.id}');{/if}{else}doDeleteTicket();{/if}"><i class="fas fa-times fa-fw"></i></a>
                            {/if}
                            <a href="#" onClick="quoteTicket('{if !$reply.id}{$ticketid}{/if}','{if $reply.id}{$reply.id}{/if}')" alt="نقل قول"><i class="fas fa-comments"></i></a>
                            <hr>
                    {if $reply.id}
                       <p><input type="checkbox" name="rids[]" value="{$reply.id}" /> انتخاب پیام </p>
                    {/if}
                        </div>
                        <div class="editbtns{if $reply.id}r{$reply.id}{else}t{$ticketid}{/if}" style="display:none">
                            <img src="../assets/img/spinner.gif" width="16" height="16" class="saveSpinner" style="display: none" />
                            <input type="button" value="{$_ADMINLANG.global.save}" onclick="editTicketSave('{if $reply.id}r{$reply.id}{else}t{$ticketid}{/if}')" class="btn btn-xs btn-small btn-primary" />
                            <input type="button" value="{$_ADMINLANG.global.cancel}" onclick="editTicketCancel('{if $reply.id}r{$reply.id}{else}t{$ticketid}{/if}')" class="btn btn-xs btn-small btn-warning" />
                        </div>
                    </div>
                </div>
                <div class="leftcol">
                            ...
                            {foreach $reply.attachments as $num => $attachment}
                                        ...
                                        <div class="ticketattachmentlinks">
                                            <small>
                                                {if $attachment.isImage}
                                                    <a class="badge badge-success" href="../{$attachment.dllink}">{lang key='support.download'}</a> 
                                                {/if}
                                                <a class="badge badge-danger" href="{$attachment.deletelink}" onclick="return confirm('{$_ADMINLANG.support.delattachment|escape:'javascript'}')">
                                ...
                                {else}
                                    ...
                                    <small>
                                        ...
                                        {/if}
                                        <a href="{$attachment.deletelink}" onclick="return confirm('{$_ADMINLANG.support.delattachment|escape:'javascript'}')">
                                            {$_ADMINLANG.support.remove}
                                        </a>
                                    </small>
                                    <br />
